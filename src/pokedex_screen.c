...
static void DexScreen_PrintMonDexNoGray(u8 windowId, u8 fontId, u16 species, u8 x, u8 y);
...
static const struct ListMenuItem sListMenuItems_KantoDexModeSelect[] = {
    ...
    {gText_AToZModeSeen,                 DEX_MODE(ATOZ)},
...
static const struct ListMenuItem sListMenuItems_NatDexModeSelect[] = {
    {gText_PokemonList,                  LIST_HEADER},
    {gText_AToZModeSeen,                 DEX_MODE(ATOZ)},
    {gText_AToZModeAll,                  DEX_MODE(TYPE)},
    {gText_PokemonHabitats,              LIST_HEADER},
    DEX_GROUP_ITEMS()
    {gText_PokedexOther,                 LIST_HEADER},
    {gText_ClosePokedex,                 LIST_CANCEL},
...
static bool8 gShowUnseen = FALSE;
...
static void Task_PokedexScreen(u8 taskId)
{
    ...
    {
    case 0:
        sPokedexScreenData->unlockedCategories = 0x7FFFFFFF;
    ...
    case 6:
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            case DEX_MODE(SMALLEST):
                if (sPokedexScreenData->modeSelectInput == DEX_MODE(ATOZ)) {
                    gShowUnseen = FALSE;
                }
                else {
                    gShowUnseen = TRUE;
                    sPokedexScreenData->modeSelectInput = DEX_MODE(ATOZ);
                }
                sPokedexScreenData->modeSelectInput = DEX_MODE(ATOZ);
                RemoveScrollIndicatorArrowPair(sPokedexScreenData->scrollArrowsTaskId);
...
static void DexScreen_InitGfxForTopMenu(void)
{
    ...
    if (IsNationalPokedexEnabled())
    {
        ...
        DexScreen_PrintNum3RightAlign(sPokedexScreenData->dexCountsWindowId, 0, sPokedexScreenData->numSeenKanto, 52, 13, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->dexCountsWindowId, FONT_SMALL, gText_National, 8, 24, 0);
        DexScreen_PrintNum3RightAlign(sPokedexScreenData->dexCountsWindowId, 0, sPokedexScreenData->numSeenNational, 52, 24, 0);
        ...
        DexScreen_PrintNum3RightAlign(sPokedexScreenData->dexCountsWindowId, 0, sPokedexScreenData->numOwnedKanto, 52, 48, 0);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->dexCountsWindowId, FONT_SMALL, gText_National, 8, 59, 0);
        DexScreen_PrintNum3RightAlign(sPokedexScreenData->dexCountsWindowId, 0, sPokedexScreenData->numOwnedNational, 52, 59, 0);
    }
    else
    {
...
static void Task_DexScreen_CharacteristicOrder(u8 taskId)
{
    ...
    case 6:
        ...
        if (JOY_NEW(A_BUTTON))
        {
            if (!DexScreen_LookUpCategoryBySpecies(sPokedexScreenData->characteristicMenuInput))
            {
                RemoveScrollIndicatorArrowPair(sPokedexScreenData->scrollArrowsTaskId);
                BeginNormalPaletteFade(~0x8000, 0, 0, 16, RGB_WHITEALPHA);
                sPokedexScreenData->state = 7;
...
static void Task_DexScreen_CategorySubmenu(u8 taskId)
{
    ...
    case 16:
        ...
        break;
    case 17:
        if (JOY_NEW(A_BUTTON) || (JOY_HELD(R_BUTTON) && DexScreen_GetSetPokedexFlag(sPokedexScreenData->dexSpecies, FLAG_GET_SEEN, TRUE)))
        {
            RemoveDexPageWindows();
    ...
    case 23:
        ...
        else if (JOY_NEW(B_BUTTON) || (JOY_NEW(R_BUTTON) && DexScreen_GetSetPokedexFlag(sPokedexScreenData->dexSpecies, FLAG_GET_SEEN, TRUE)))
        {
        ...
        {
            if (JOY_NEW(R_BUTTON) && !DexScreen_GetSetPokedexFlag(sPokedexScreenData->dexSpecies, FLAG_GET_SEEN, TRUE))
            {
                PlaySE(SE_FAILURE);
            }
            DexScreen_InputHandler_StartToCry();
...
static void DexScreen_PrintMonDexNoGray(u8 windowId, u8 fontId, u16 species, u8 x, u8 y)
{
    ...
    DexScreen_AddTextPrinterParameterized(windowId, fontId, gText_PokedexNo, x, y, TEXT_COLOR_DARK_GRAY);
    DexScreen_PrintNum3LeadingZeroes(windowId, fontId, dexNum, x + 9, y, TEXT_COLOR_DARK_GRAY);
}
...
bool8 DexScreen_DrawMonPicInCategoryPage(u16 species, u8 slot, u8 numSlots)
{
    ...
    if (sPokedexScreenData->categoryMonInfoWindowIds[slot] == 0xFF)
    {
        if (species != SPECIES_NONE)
        {
            ...
            {
                // DARK_GRAY prints red for some reason, but it looks better that way.
                DexScreen_PrintMonDexNoGray(sPokedexScreenData->categoryMonInfoWindowIds[slot], FONT_SMALL, species, 12, 0);
                DexScreen_AddTextPrinterParameterized(sPokedexScreenData->categoryMonInfoWindowIds[slot], FONT_NORMAL, gSpeciesNames[species], 2, 13, TEXT_COLOR_DARK_GRAY);
            }
            else
            {
...
static bool8 DexScreen_CreateCategoryListGfx(bool8 justRegistered)
{
    ...
    if (justRegistered)
    {
        DexScreen_PrintStringWithAlignment(gGroupNamePtrs[sPokedexScreenData->category], TEXT_CENTER);
    }
    else
    {
        DexScreen_PrintStringWithAlignment(gGroupNamePtrs[sPokedexScreenData->category], TEXT_LEFT);
...
u8 DexScreen_DrawMonAreaPage(void)
{
    ...
    u16 kantoMapVoff;
    u8 birdsFought = FlagGet(FLAG_FOUGHT_ARTICUNO) + FlagGet(FLAG_FOUGHT_ZAPDOS) + FlagGet(FLAG_FOUGHT_MOLTRES);
    ...
    {
        if (JOY_HELD(R_BUTTON) && DexScreen_GetSetPokedexFlag(species, FLAG_GET_SEEN, TRUE))
        {
            if (birdsFought == 0)
            {
                s32 strWidth = GetStringWidth(FONT_SMALL, gText_PreciseArea, 0);
                DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_PreciseArea, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 2, 4, 0);
            }
            if (birdsFought == 1)
            {
                s32 strWidth = GetStringWidth(FONT_SMALL, gText_PreciseArea1, 0);
                DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_PreciseArea1, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 2, 4, 0);
            }
            if (birdsFought == 2)
            {
                s32 strWidth = GetStringWidth(FONT_SMALL, gText_PreciseArea2, 0);
                DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_PreciseArea2, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 2, 4, 0);
            }
            if (birdsFought == 3)
            {
                s32 strWidth = GetStringWidth(FONT_SMALL, gText_PreciseArea3, 0);
                DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_PreciseArea3, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 2, 4, 0);
            }
        }
        else
        {
            ...
            s32 strWidth = GetStringWidth(FONT_SMALL, gText_PossibleArea, 0);
            DexScreen_AddTextPrinterParameterized(sPokedexScreenData->windowIds[10], FONT_SMALL, gText_PossibleArea, (sWindowTemplate_AreaMap_Area.width * 8 - strWidth) / 2, 4, 0);
        }
    }
...
{
    u16 i;
    const u16 currPageStart = 4 * pageNum + 1;
    const u16 nextPageStart = 4 * (pageNum + 1) + 1;
    const u16 count = gGroupPtrs[categoryNum][0];

    sPokedexScreenData->numMonsOnPage = 0;

    for (i = 0; i < 4; i++)
        sPokedexScreenData->pageSpecies[i] = 0xffff;
    for (i = currPageStart; i < nextPageStart && i <= count; i++)
    {
        sPokedexScreenData->pageSpecies[sPokedexScreenData->numMonsOnPage] = gGroupPtrs[categoryNum][i];
...
static u8 DexScreen_GetPageLimitsForCategory(u8 category)
{
    ...
    if (lastPage != 0xff)
    {
        ...
        sPokedexScreenData->lastPageInCategory = gGroupPtrs[category][0] / 4 + (gGroupPtrs[category][0] % 4 != 0);
        return FALSE;
    }
    else
    {
...
static u8 DexScreen_LookUpCategoryBySpecies(u16 species)
{
    ...
    for (i = 0; i < NELEMS(gGroupPtrs); i++)
    {
        for (j = 0; j <= gGroupPtrs[i][0]; j++)
        {
            if (gGroupPtrs[i][j] == species)
            {
                sPokedexScreenData->category = i;
                sPokedexScreenData->pageNum = (j-1) / 4;
                sPokedexScreenData->categoryCursorPosInPage = (j-1) % 4;
                return FALSE;
            }
        }
    }
