...
static bool32 IsSpeciesPossiblyInEncounterTable(const struct WildPokemonHeader * data, const struct WildPokemonInfo * info, s32 species, s32 count, u8 area);
...
static const struct RoamerPair sRoamerPairs[] = {
    { SPECIES_TANGROWTH,   SPECIES_BULBASAUR  },
    { SPECIES_YANMEGA, SPECIES_CHARMANDER },
    { SPECIES_ROSERADE,  SPECIES_SQUIRTLE   }
};
...
static void FillPossibleSpecies(
    const struct WildPokemonHeader * data, 
    s32 species, 
    s32 * earlyBoostSpecies,
    s32 * groupSpecies1, 
    s32 * groupSpecies2) 
{
    u32 mapHash = MapHashFromMapId(FALSE, data->mapGroup, data->mapNum);
  
    const u16* group = gMonGroups[species];

    u32 tempCombinedHash1 = HashCombine(GameHash(), mapHash * species);
    u32 tempCombinedHash2 = HashCombine(mapHash, GameHash() * species);
    ...
    if (BadgeCount() != 8 && 
        group != gGroup_WaterIce1 && 
        group != gGroup_WaterIce2) {
        *earlyBoostSpecies = IndexInto(gGroup_EarlyBoost, HashCombine(GameHash(), mapHash));
    } else {
        *earlyBoostSpecies = -1;
    }

    *groupSpecies1 = IndexInto(group, tempCombinedHash1);
    *groupSpecies2 = IndexInto(group, tempCombinedHash2);
}
...
static bool32 IsSpeciesOnMap(const struct WildPokemonHeader * data, s32 species)
{
    if (JOY_HELD(R_BUTTON) && DexScreen_GetSetPokedexFlag(species, FLAG_GET_SEEN, TRUE))
    {
        ...
        u32 mapHash = MapHashFromMapId(FALSE, data->mapGroup, data->mapNum);
        if (birdsFought <= HashCombine(GameHash(), mapHash * species) % 3)
        {
            return FALSE;
        }
    ...
    // As a result it's reading out of bounds of the fishing encounters tables.
        ...
        if (IsSpeciesPossiblyInEncounterTable(data, data->landMonsInfo, species, LAND_WILD_COUNT, WILD_AREA_LAND))
            return TRUE;
        if (IsSpeciesPossiblyInEncounterTable(data, data->waterMonsInfo, species, WATER_WILD_COUNT, WILD_AREA_WATER))
            return TRUE;
    // When searching the fishing encounters, this incorrectly uses the size of the land encounters.
    // As a result it's reading out of bounds of the fishing encounters tables.
#ifdef BUGFIX
        if (IsSpeciesPossiblyInEncounterTable(data, data->fishingMonsInfo, species, FISH_WILD_COUNT, WILD_AREA_FISHING))
#else
        if (IsSpeciesPossiblyInEncounterTable(data, data->fishingMonsInfo, species, LAND_WILD_COUNT, WILD_AREA_FISHING))
#endif
            return TRUE;
        if (IsSpeciesPossiblyInEncounterTable(data, data->rockSmashMonsInfo, species, ROCK_WILD_COUNT, WILD_AREA_ROCKS))
            return TRUE;
    }
